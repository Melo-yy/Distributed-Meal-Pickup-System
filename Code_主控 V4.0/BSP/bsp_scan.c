...
#include "FreeRTOS.h"
#include "semphr.h"
extern SemaphoreHandle_t xSemaphoreScan;
        ...
        if (recv_ok == 0)               // 上一帧未取走则丢弃新字节
        {
            if (recv_len < UART_BUF_LEN - 1)   // 留 1 字节给 '\0' 或 '\n'
            {
                ...
                if (d == 0x0D)          // 收到 CR，认为一帧结束
                {
                    ...
                    recv_ok = 1;
                    /* 释放信号量通知 scan_task：有扫码帧到达，不用轮询了 */
                    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
                    xSemaphoreGiveFromISR(xSemaphoreScan, &xHigherPriorityTaskWoken);
                    // 如果唤醒了更高优先级任务，立即切换过去执行
